...
void DeclareSubMiniTable(upb::FieldDefPtr field, const DefPoolPair& pools,
  ...
  if (seen.insert(pools.GetMiniTable64(field.message_type())).second) {
    ...
    // "default" empty MiniTable instead of having each leaf define its own.
    // This will reduce binary size if many messages are tree shaken.
    output(R"cc(
    ...
    output("\n");
    // This currently only works with Apple platforms. Once we are ready to fix
    // b/417242115, we should make this work on Linux also.
    output(
        R"cc(
...
void WriteMessage(upb::MessageDefPtr message, const DefPoolPair& pools,
  ...
  for (int i = 0; i < mt_64->UPB_PRIVATE(field_count); i++) {
    ...
    auto pair = subs.emplace(index, GetSub(field, false, options));
